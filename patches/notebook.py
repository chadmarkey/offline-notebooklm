...
# --- Infinity reranker patch (env-gated) --------------------------------------
# When RERANK_ENABLED=true, vector_search over-fetches candidates, sends them to the
# Infinity bge-reranker, and returns the best-reordered top-N. Best-effort: any reranker
# failure falls back to vector order, so retrieval never breaks.
# RE-APPLY NOTE: this lives in a bind-mounted copy of domain/notebook.py. If the
# open_notebook image tag is bumped, re-copy the file from the new image and re-apply
# this block. See ~/offline-notebook/README.md.
async def _rerank(query: str, results: list, top_n: int) -> list:
    if len(results) <= 1:
        return results[:top_n]
    docs = []
    for r in results:
        m = r.get("matches")
        docs.append(" ".join(m) if isinstance(m, list) else str(m or r.get("title") or ""))
    url = os.getenv("RERANK_URL", "http://infinity:7997/v1/rerank")
    ...
    try:
        ...
        ordered = [results[i["index"]] for i in ranked if i.get("index") is not None]
        return ordered or results[:top_n]
    except Exception as e:
        logger.warning(f"Reranker unavailable, using vector order: {e}")
        return results[:top_n]
...
async def vector_search(
    ...
    try:
        ...
        fetch_count = (
            max(results, int(os.getenv("RERANK_FETCH", "20"))) if rerank else results
        )
        ...
        if rerank and search_results:
            top_n = min(results, int(os.getenv("RERANK_TOPN", "5")))
            return await _rerank(keyword, search_results, top_n)
        return search_results
    except Exception as e:
